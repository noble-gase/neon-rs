...
impl NacosConfig {
    ...
    pub async fn shutdown(&mut self) -> Result<()> {
        ...
        remote.reload_worker.abort();
        let _ = remote.reload_worker.await;
...
impl ConfigBuilder {
    ...
    async fn build_remote(self, env: Env) -> Result<NacosConfig> {
        ...
        for data_id in &merge_order {
            let resp = service
                .get_config(data_id.clone(), source.group.clone())
                .await?;
        ...
        let reload_worker = tokio::spawn(async move {
            run_reload_worker(worker_listener, update_rx).await;
        });
...
/// 按 data-id 顺序合并多个配置；后者覆盖前者同名键。缺少条目的 data-id 会被跳过。
fn merge_configs(
    data_id_order: &[String],
    entries: &HashMap<String, ConfigEntry>,
) -> Result<Config> {
    let mut builder = Config::builder();
    let mut merged_any = false;
    for data_id in data_id_order {
        let Some(entry) = entries.get(data_id) else {
            continue;
        };
        merged_any = true;
...
/// nacos 配置变更监听器：异步队列 coalesce 后单次合并，再推送到 watch 通道。
...
impl ReloadListener {
    fn enqueue_update(&self, data_id: String, content: String, format: Format) {
        if self
            ...
            .send(PendingUpdate {
                data_id,
                entry: ConfigEntry { content, format },
            })
            .is_err()
        {
            tracing::warn!("配置热更新 worker 已停止，丢弃变更");
        }
    }
...
async fn run_reload_worker(
    listener: Arc<ReloadListener>,
    mut update_rx: mpsc::UnboundedReceiver<PendingUpdate>,
) {
    while let Some(first) = update_rx.recv().await {
        let mut batch = HashMap::new();
        batch.insert(first.data_id, first.entry);
        while let Ok(update) = update_rx.try_recv() {
            batch.insert(update.data_id, update.entry);
        }
...
"#
    ...
    #[test]
    fn merge_configs_skips_missing_entries() {
        let mut entries = HashMap::new();
        entries.insert(
            "app.toml".to_string(),
            ConfigEntry {
        ...
        let cfg = merge_configs(
            &["app.toml".to_string(), "optional.toml".to_string()],
            &entries,
        )
        .expect("should merge available entries");

        assert_eq!(cfg.get_string("name").unwrap(), "app");
    }
