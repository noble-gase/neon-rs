...
/// ```
pub async fn insert<'e, E>(db: E, stmt: InsertStatement) -> anyhow::Result<InsertResult<u64>>
where
    E: Executor<'e, Database = MySql>,
{
    let (sql, values) = stmt.build_sqlx(MysqlQueryBuilder);

    let log_sql = log_sql(|| inject_parameters(&sql, &values.0.0, &MysqlQueryBuilder));
    ...
    T: for<'r> FromRow<'r, MySqlRow> + Send + Unpin,
{
    ...
    let log_count_sql =
        log_sql(|| inject_parameters(&count_sql, &count_values.0.0, &MysqlQueryBuilder));
    ...
    let log_query_sql =
        log_sql(|| inject_parameters(&query_sql, &query_values.0.0, &MysqlQueryBuilder));
